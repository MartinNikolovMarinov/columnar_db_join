...
/*
    ...
    literarily included in every other file. This code will likely be used in a 'using namspace core' statement,
...
static_assert(sizeof(addr_size) == 8, "addr_size is not 8 bytes");
static_assert(sizeof(addr_off) == 8, "addr_off is not 8 bytes");

static_assert(sizeof(rune) == 4, "rune is not 4 bytes");
static_assert(sizeof(uchar) == 1, "uchar is not 1 byte");
static_assert(sizeof(schar) == 1, "schar is not 1 byte");
